...
int vce_siti_main(int argc, char **argv)
{
    ...
    options.add_options()("i,input-file", "input: images (BMP, TIFF, PNG, JPEG...), video (.avi, .mkv, .mp4, .yuv... )", cxxopts::value<std::string>())("w,width", "Width of the video (required for yuv).", cxxopts::value<int>())("h,height", "Height of the video (required for yuv).", cxxopts::value<int>())("f,color-format", "(int) Color representation of YUV format (1: YUV420p (default), 2: YUV422, 3: YUYV422 (YUY2), 4: YUYV422 (UYVY), 5: YUV444.", cxxopts::value<int>())("s,summary", "produce summary statistics (maximum, minimum) instead of per-frame information")("c,check-input", "Show in a window how frames are read");
